...
use crate::config::{Config, PoolConfig, WalletConfig};
...
impl LolMinerAdapter {
    pub fn new() -> Self {
        ...
        let shares_pattern = Regex::new(r"Accepted:\s*(\d+),\s*Rejected:\s*(\d+)")
            .expect("Invalid shares regex");
        let temp_pattern = Regex::new(r"GPU\s+\d+.*?(\d+)°C")
            .expect("Invalid temperature regex");
...
impl MinerAdapter for LolMinerAdapter {
    ...
    fn parse_telemetry_line(&self, line: &str) -> Option<Telemetry> {
        ...
        telemetry.algorithm = "ethash".to_string();
        ...
        if let Some(captures) = self.shares_pattern.captures(line) {
            if let (Some(accepted_match), Some(rejected_match)) = (captures.get(1), captures.get(2)) {
                if let (Ok(accepted), Ok(rejected)) = 
                    (accepted_match.as_str().parse::<u32>(), rejected_match.as_str().parse::<u32>()) {
    ...
    async fn verify_binary(&self, binary_path: &Path) -> Result<()> {
        ...
        if !metadata.is_file() {
            return Err(anyhow!("Binary path is not a file: {}", binary_path.display()));
        }
    ...
    async fn download_binary(&self, download_dir: &Path) -> Result<PathBuf> {
        ...
        let binary_name = if cfg!(windows) { "lolMiner.exe" } else { "lolMiner" };
...
impl XMRigAdapter {
    pub fn new() -> Self {
        let hashrate_pattern = Regex::new(r"(\d+\.?\d*)\s*(H/s|kH/s|MH/s)")
            .expect("Invalid hashrate regex");
        let shares_pattern = Regex::new(r"accepted:\s*(\d+)/(\d+)")
            .expect("Invalid shares regex");
        let temp_pattern = Regex::new(r"temp:\s*(\d+)C")
            .expect("Invalid temperature regex");
...
impl MinerAdapter for XMRigAdapter {
    ...
    fn parse_telemetry_line(&self, line: &str) -> Option<Telemetry> {
        let mut telemetry = Telemetry::default();
        telemetry.algorithm = "randomx".to_string();
        let mut updated = false;
        ...
        if let Some(captures) = self.shares_pattern.captures(line) {
            if let (Some(accepted_match), Some(total_match)) = (captures.get(1), captures.get(2)) {
                if let (Ok(accepted), Ok(total)) = 
                    (accepted_match.as_str().parse::<u32>(), total_match.as_str().parse::<u32>()) {
                    telemetry.shares_accepted = accepted;
...
impl MinerManager {
    ...
    pub async fn ensure_binary_available(&self, adapter: &Arc<dyn MinerAdapter>) -> Result<PathBuf> {
        ...
        match adapter.verify_binary(&binary_path).await {
            Ok(_) => {
                debug!("Binary {} verified at {}", binary_info.name, binary_path.display());
        ...
        fs::create_dir_all(&binary_dir)
            .context("Failed to create binary directory")?;
        
        // Download binary
        info!("Downloading {} binary...", binary_info.name);
        let downloaded_path = adapter.download_binary(&binary_dir).await
            .context("Failed to download binary")?;
        
        // Verify downloaded binary
        adapter.verify_binary(&downloaded_path).await
            .context("Downloaded binary verification failed")?;
        
        info!("✓ Binary {} downloaded and verified", binary_info.name);
...
impl ProcessSupervisor {
    ...
    pub async fn start(&mut self, telemetry_sender: mpsc::UnboundedSender<Telemetry>) -> Result<()> {
                ...
                match child.try_wait() {
                    Ok(Some(exit_status)) => {
                        ...
                        if exit_status.success() {
                            ...
                            error!("Miner process crashed with exit code: {:?}", exit_status.code());
                            ...
                            if self.restart_count < self.max_restart_attempts {
                                ...
                                info!("Restarting miner in {} seconds (attempt {}/{})", 
                                     delay, self.restart_count + 1, self.max_restart_attempts);
    ...
    async fn spawn_process(&mut self) -> Result<()> {
        let args = self.adapter.build_args(&self.config, &self.device_ids)
            .context("Failed to build miner arguments")?;
        
        info!("Spawning miner: {} {}", self.binary_path.display(), args.join(" "));
...
mod tests {
    ...
    fn test_lolminer_args_generation() {
        ...
        assert!(args.iter().any(|arg| arg.contains("742d35Cc6635C0532925a3b8D400cdFb7021f39f")));
